...
def test_arbitrary_dv_shape():
    m = CausalSelfAttention(heads=4, d_model=32, d_v=16, dropout=0.0)
    x = torch.randn(3, 5, 32)
    out = m(x)
    assert out.shape == (3, 5, 32)

def test_single_head():
    m = CausalSelfAttention(heads=1, d_model=64, d_v=64, dropout=0.0)
    x = torch.randn(1, 8, 64)
    out = m(x)
    assert out.shape == (1, 8, 64)

def test_large_batch_long_sequence():
    m = CausalSelfAttention(heads=8, d_model=512, d_v=64, dropout=0.0)
...
def test_invalid_d_model_divisibility():
    with pytest.raises(AssertionError):
        CausalSelfAttention(heads=6, d_model=32, d_v=8)
...
def test_causal_masking_blocks_future():
    ...
    m = CausalSelfAttention(heads=2, d_model=16, d_v=8, dropout=0.0)
...
def test_deterministic_with_fixed_seed():
    ...
    m1 = CausalSelfAttention(heads=4, d_model=32, d_v=8, dropout=0.0)
    x = torch.randn(2, 10, 32)
    out1 = m1(x)

    torch.manual_seed(42)
    m2 = CausalSelfAttention(heads=4, d_model=32, d_v=8, dropout=0.0)
    out2 = m2(x)
...
def test_dropout_effect_in_training_mode():
    m = CausalSelfAttention(heads=4, d_model=32, d_v=8, dropout=0.5)
    m.train()
    x = torch.randn(2, 10, 32)
...
def test_very_small_d_v():
    m = CausalSelfAttention(heads=4, d_model=32, d_v=2, dropout=0.0)
...
def test_parameter_count():
    m = CausalSelfAttention(heads=4, d_model=64, d_v=16, dropout=0.0)
